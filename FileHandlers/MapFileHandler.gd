...
func _init(file_path):
	...
	for i in range(tile_count):
		...
		var ab_index: int = file_bytes.decode_u16(file_position + idx)
		var unpassable_tile: bool = bool(file_bytes.decode_u16(file_position + idx + 2)) if includes_passable_tiles else false
		var sobj_offset: int = 4 if includes_passable_tiles else 2
		var sobj_index: int = file_bytes.decode_u16(file_position + idx + sobj_offset) - 1
		tiles[i] = MapTile.new(
			ab_index,
			unpassable_tile,
			sobj_index,
